...
from ProcessData.process_data_constants import PRCC, STREET42
...
def CTL_data_loader(config):
    ...
    if config.DATASETS.NAMES == STREET42:
        galleryloader = DataLoader(dataset=Street42Dataset(dataset.gallery, transform=transform_test),
    ...
    elif config.DATASETS.NAMES == STREET42:
        queryloader = DataLoader(dataset=Street42Dataset(dataset.query, transform=transform_test),
...
def CAL_data_loader(config):
    ...
    if config.DATA.DATASET == STREET42:
        galleryloader = DataLoader(dataset=Street42Dataset(dataset.gallery, transform=transform_test),
                                   sampler=SequentialSampler(dataset.gallery),
    ...
    if config.DATA.DATASET == PRCC:
        ...
        return queryloader_same, queryloader_diff, galleryloader, dataset, enrichedloader
    elif config.DATA.DATASET == STREET42:
        queryloader = DataLoader(dataset=Street42Dataset(dataset.query, transform=transform_test),
                                 sampler=SequentialSampler(dataset.query),
                                 batch_size=config.DATA.TEST_BATCH, num_workers=config.DATA.NUM_WORKERS,
                                 pin_memory=True, drop_last=False, shuffle=False)
