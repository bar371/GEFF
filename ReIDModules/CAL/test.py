...
@torch.no_grad()
def extract_img_feature(model, dataloader, device='cuda'):
    features, pids, camids, clothes_ids = [], torch.tensor([]), torch.tensor([]), torch.tensor([])
    for batch_idx, (imgs, batch_pids, batch_camids, batch_clothes_ids) in tqdm.tqdm(enumerate(dataloader),total=len(dataloader)):
...
def extract_vid_feature(model, dataloader, vid2clip_index, data_length):
    ...
    print('Extracting vid features from data')
    ...
    print('Some extra vid processing for you')
    ...
    for i, idx in tqdm.tqdm(enumerate(vid2clip_index),total=len(vid2clip_index)):
    ...
    print('Done feature extraction by video')
    return features, pids, camids, clothes_ids
